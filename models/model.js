...
const model = {
  async mergeTokenLists(req, res, next) {
    try {
      ...
      await RedisClient.set("baseAssets", JSON.stringify(removedDuplpicates));
  ...
  async updateAssets(req, res, next) {
    try {
      if (config.testnet === "1") {
        ...
        await RedisClient.set("baseAssets", JSON.stringify(tokenList));
  ...
  async _getAssetPrices(tokenList, pairs) {
    try {
      const url = `https://api.covalenthq.com/v1/pricing/tickers/?quote-currency=USD&format=JSON&tickers=USDC,FTM&key=${process.env.COVALENT_KEY}`;
      ...
      const tokenListWithPrices = tokenList.map((token) => {
        ...
        if (
          ...
          if (
            ...
            config.wftm.address.toLowerCase()
          ) {
  ...
  getRouteAssets(req, res, next) {
    try {
      const routeAssets = [config.wftm, config.solidSEX];
      res.status(205);
  ...
  async updatePairs(req, res, next) {
    try {
      ...
      await RedisClient.set("pairs", JSON.stringify(ps));
  ...
  async _getBaseAsset(web3, address) {
    try {
      ...
      await RedisClient.set("extraAssets", JSON.stringify(extraAssets));
